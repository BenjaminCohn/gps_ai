...
async function loadStationsInBbox(minLat, maxLat, minLng, maxLng) {
  try {
    ...
    // 1. Récupérer les stations OSM (localisation)
    const stations = await fetchStationsFromOSM(minLat, maxLat, minLng, maxLng);

    // 2. Récupérer les prix depuis l'API gouvernementale
    ...
    if (merged.length > 0) showToast(`${merged.length} stations trouvées`);
  } catch (err) {
...
async function fetchPricesGouvernement(minLat, maxLat, minLng, maxLng) {
  // API officielle gouvernement français - prix carburants temps réel
  const centerLat = ((minLat + maxLat) / 2).toFixed(4);
  const centerLng = ((minLng + maxLng) / 2).toFixed(4);

  // Tentative 1 : API instantanée officielle
  try {
  ...
  // Tentative 2 : API data.gouv JSON
  try {
  ...
  // Tentative 3 : Proxy CORS open data
  try {
  ...
  return []; // Pas de prix disponibles — stations OSM sans prix
}
...
function mergeStationsAndPrices(osmStations, govData) {
  ...
  govData.forEach(gov => {
    // Coordonnées gouvernementales (parfois x100000)
    ...
    // Trouver la station OSM la plus proche
    let best = null, bestDist = 0.5; // max 500m
    ...
    if (best) {
      ...
      // Ajouter la station gouvernementale si pas dans OSM
      osmStations.push({
  ...
  if (Array.isArray(fuels)) {
    fuels.forEach(f => {
      ...
      if (name && val) {
        ...
        prices[name] = price > 10 ? price / 1000 : price; // normaliser si en millimes
      }
    });
  }
  // Format API v2 : objet direct
...
function renderStationMarkers(stations) {
  stations.forEach(s => {
    ...
    el.style.cssText = `
      background:${brand.bg};border:2px solid ${brand.color};
      border-radius:10px;padding:4px 6px 2px;
      display:flex;flex-direction:column;align-items:center;gap:1px;
      cursor:pointer;min-width:36px;text-align:center;
      filter:drop-shadow(0 2px 6px rgba(0,0,0,0.4));
      transition:transform 0.15s;font-family:'Syne',sans-serif;
    `;
    el.innerHTML = `
    ...
    el.addEventListener('mouseenter', () => el.style.transform = 'scale(1.1) translateY(-3px)');
    el.addEventListener('mouseleave', () => el.style.transform = '');
    el.addEventListener('click', (e) => { e.stopPropagation(); openStationPopup(s); });
...
function openStationPopup(station) {
  ...
  const distStr = userLocation
    ? (() => { const d = haversineKm(userLocation.lat, userLocation.lng, station.lat, station.lng); return d < 1 ? Math.round(d*1000)+'m' : d.toFixed(1)+'km'; })()
    : '';

  const serviceIcons = {
    'Toilettes':'🚻','Boutique':'🛍','Restaurant':'🍔','Lavage':'🚗💧','Wifi':'📶'
  };
  const servicesHTML = (station.services || [])
    .map(s => serviceIcons[s] || null).filter(Boolean).join(' ');

  const fuelsHTML = hasPrices
    ? Object.entries(prices).map(([fuel, price]) => {
        const info = FUEL_LABELS[fuel] || { label: fuel, icon: '⛽', color: '#6B7280' };
        // Colorer selon le prix (vert=bon, orange=moyen, rouge=cher)
  ...
  const html = `
    <div style="font-family:'Syne',sans-serif;background:#0a0f1e;border-radius:18px;overflow:hidden;width:270px;border:0.5px solid rgba(255,255,255,0.1)">
      ...
      <div style="display:flex;gap:8px;padding:12px 16px">
        <button onclick="navigateToStation(${station.lat},${station.lng},'${(station.name||'').replace(/'/g,'')}')" style="flex:1;background:linear-gradient(135deg,#0050cc,#003db5);border:none;border-radius:10px;padding:10px;font-family:'Syne',sans-serif;font-size:13px;font-weight:700;color:white;cursor:pointer">→ Y aller</button>
  ...
  const popup = new mapboxgl.Popup({
    ...
    maxWidth: '290px', anchor: 'bottom', offset: [0, -52],
  })
  ...
  // Ne pas bouger la carte — juste centrer légèrement sans changer le zoom
  // pour ne pas perturber la navigation
  if (!navActive) {
...
function toggleStations() {
  ...
  stationMarkers.forEach(({ el }) => el.style.display = stationsVisible ? '' : 'none');
